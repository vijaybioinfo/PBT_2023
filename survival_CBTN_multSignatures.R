...
# # Clinical Data
# mdata_cBioPortal <- read.csv("~/Documents/openpbta_clinical_data_cBioPortal.tsv", sep = "\t") %>% filter(TUMOR_TYPE == "primary")
# mdata_CAVATICA <- read.csv("~/Documents/openpbta_clinical_data_CAVATICA.tsv", sep = "\t") %>% filter(Tumor.Descriptor == "Initial CNS Tumor")
# 
# mdata_cBioPortal <- mdata_cBioPortal[,c("Patient.ID", "GENDER", "AGE", "OS_STATUS", "OS_MONTHS", "TUMOR_TYPE", "CANCER_TYPE_DETAILED")]; colnames(mdata_cBioPortal) <- c("Patient.ID", "Sex", "age_at_diagnosis", "OS_STATUS", "OS_MONTHS", "TUMOR_TYPE", "disease_type") # , "Sample.ID"
# mdata_CAVATICA <- mdata_CAVATICA[,c("Kids.First.Participant.ID", "gender", "age_at_diagnosis", "Tumor.Descriptor", "disease_type")]; colnames(mdata_CAVATICA) <- c("Kids.First.Participant.ID", "Sex", "age_at_diagnosis", "TUMOR_TYPE", "disease_type")
# mdata <- merge(mdata_cBioPortal, mdata_CAVATICA, by.x = "Patient.ID", by.y = "Kids.First.Participant.ID", suffixes = c(".cBioPortal",".CAVATICA"), all.x = T) %>% distinct(); dim(mdata)
# mdata$VITAL_STATUS = ifelse(mdata$OS_STATUS == '0:LIVING', 0, 1)
# 
# mdata <- mdata[!is.na(mdata$Sex.cBioPortal) & mdata$Sex.cBioPortal %in% c("Male", "Female") & !is.na(mdata$age_at_diagnosis.cBioPortal) & !is.na(mdata$OS_MONTHS) & !is.na(mdata$OS_STATUS),] %>% distinct(); dim(mdata)
# mdata_ <- mdata
# 
# mdata <- mdata[,!colnames(mdata) %in% c("age_at_diagnosis.CAVATICA", "disease_type.CAVATICA", "Sex.CAVATICA", "disease_type.cBioPortal")] %>% distinct()
# rownames(mdata) <- mdata$Patient.ID
# # mdata[duplicated(mdata$Patient.ID),]
...
table(mdata_surv$SIGNATURE_GROUP)
...
# Correcting for stage
fit <- coxph(Surv(time = mdata_surv$OS_MONTHS, event = mdata_surv$VITAL_STATUS) ~ SIGNATURE_GROUP + AGE + GENDER + ANALYSIS, data = mdata_surv) # Looks better withou adding strata
ggadjustedcurves(fit, data = mdata_surv, method = "average", variable = "SIGNATURE_GROUP") # , palette = c("#E7B800", "#2E9FDF")
ggforest(fit)

table(mdata_surv$VITAL_STATUS, mdata_surv$SIGNATURE_GROUP)

# ----------------------------------------

table(mdata$CANCER_TYPE_DETAILED, mdata$NeoTCR8_group)

table(mdata$ANALYSIS, mdata$NeoTCR4_group)
tmp <- mdata %>% filter(NeoTCR4_group == "low")
sort(table(tmp$CANCER_TYPE_DETAILED))

lapply(grep("group", colnames(mdata), value=T), function(x) table(mdata[,x]))
# ----------------------------------------


## Create Metadata
mdata_cBioPortal <- read.csv("~/Documents/openpbta_clinical_data_cBioPortal.tsv", sep = "\t")
mdata_cBioPortal_filt <- mdata_cBioPortal %>% filter(SAMPLE_TYPE == "Solid Tissue" & !is.na(OS_STATUS) & !is.na(OS_MONTHS))
mdata_cBioPortal_filt$VITAL_STATUS = ifelse(mdata_cBioPortal_filt$OS_STATUS == '0:LIVING', 0, 1)
vars <- c("Patient.ID", "CANCER_TYPE", "CANCER_TYPE_DETAILED", "pathology_free_text_diagnosis", "HISTOLOGY", "TUMOR_TYPE", "OS_MONTHS", "OS_STATUS")
rest_vars <- setdiff(colnames(mdata_cBioPortal_filt), vars)
mdata_cBioPortal_filt <- mdata_cBioPortal_filt[, c(vars, rest_vars)]
write.table(mdata_cBioPortal_filt, "~/Documents/CBTN_cBioPortal_Survival_Analysis_table_noCellLines_wSurvivalData.tsv", row.names = F, sep = "\t")

unique(mdata_cBioPortal_filt$CANCER_TYPE)
