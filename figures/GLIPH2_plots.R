...
# CD8
if(celltype == "CD8"){
  min_dotsize <- 3; max_dotsize <- 8
  legend_breaks <- c(2,20,40,60) # Do not include 1
  color_legend_breaks <- c(2,6,14) # Do not include 1
...
input.path = paste0("/home/kmlanderos/tmp_large/pbtumor-all-Batch2/results/tcr/preproces_4_Gliph2/CD3p/ag_specificity_assessment/reports/ag_specificity_assessment_",date)
results.path = paste0('/home/kmlanderos/tmp_large/pbtumor-all-Batch2/results/tcr/gliph/',date); dir.create(results.path, recursive=T)
...
check.gliph2.output <- function(file.suffix, beta.info, hinojosa = FALSE, min_dotsize = 2, max_dotsize = 10, legend_breaks = NULL, cs_upper_limit = NULL, color_legend_breaks = NULL){
  ...
  if(hinojosa){
    ...
    motif_donor <- apply(gliph.res.tmp, 1, function(x){
      ...
      # BT23 and BT23;NA should not be considered as public
      # if(suma==0) donor <- "LOL"; cat(suma, "\n")
  ...
  # NOTE: Using the median results problematic when there are SG with multiple clones with clone size of 1 and 1 or a few of them whose clone size is high.
  ...
  if(hinojosa){
    ...
    # colrs.v <- c(non_public="gold", public="tomato")
    # V(igraph.obj)$color = colrs.v[V(igraph.obj)$donor]# Get general things for plotting.
  ...
  igraph.obj <- simplify(igraph.obj, remove.multiple=T, edge.attr.comb="first")

  # Plot.
  tmp.file.name <- paste0(results.path, '/GLIPH2Results_iGraph_', file.suffix, '.pdf')
  pdf(file=tmp.file.name, 10, 7)
  plot(igraph.obj, layout=layout_with_kk, alpha=0.6, vertex.label=NA, main= paste0("TCR motifs found by GLIPH2 in ", celltype, "+ Tcells")) #, vertex.label.cex = 0.6, vertex.label.dist=2, layout=layout_with_kk
