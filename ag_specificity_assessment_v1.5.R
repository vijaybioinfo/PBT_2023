...
### Running Example
# Rscript3 ag_specificity_assessment_v1.5.R --celltype CD8 --date 2022-07-15 --LG FALSE --hinojosa FALSE --iGraph FALSE --expansionThold 1 --HTO FALSE
# Rscript3 ag_specificity_assessment_v1.5.R --celltype CD8 --date 2022-07-15 --LG FALSE --hinojosa FALSE --iGraph TRUE --expansionThold 1 --HTO FALSE
...
if(iGraph){
  ...
  # Previous way of calling expanded SGs
  # exp_sg <- lapply(gliph_outs, function(x) {
  #   tmp <- x %>% select(index, pattern, TcRb, Sample, Freq) %>% group_by(index) %>% mutate(sg_cs = sum(Freq), nTCR = n()) %>% # %>% as.data.frame() %>% head(100)
  #     select(index, sg_cs, nTCR) %>% distinct()
  #   nrow(tmp[tmp$sg_cs > tmp$nTCR,])
  # }); names(exp_sg) <- suffix
  # exp_sg
...
if(!iGraph){
  ...
  fname <- paste0("/GLIPH2Input_TCR-Data_All-TRB_", hto_suffix, celltype, ".tsv")
